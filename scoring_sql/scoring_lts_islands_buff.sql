...
parkdistance as (
    ...
    from
        rs_cycle_buffer_new ra,
        rs_dvrpc_parks_and_openspace_new rpo
--    where 
...
circuitdistance as (
    ...
    from
        ...
        rs_circuittrail rc
    where 
--    	st_dwithin(ra.shape, rc.shape, 5000) and 
...
select
    ...
    civic_cnt as CIV_SM_C, --total number of civic resources within 1/2 mile : NEEDS QUANTILE
    civic_score as CIV_SC_C, --civic resources quantile score
    park_score as POS_SC_C, --parks/open space < 1/4 mile (5), 1/4 to 1/2 mile (4), 1/2 to 1 mile (3), 1-2 miles (2), > 2miles (1) 
    retail_score as WRC_SC_C, --walkable retail < 1/4 mile (5), 1/4 to 1/2 mile (4), 1/2 to 1 mile (3), 1-2 miles (2), > 2miles (1) 
    eta_cnt as ESS_SM_C, --total number of eta services within 1/2 mile : NEEDS QUANTILE
    eta_score as ESS_SC_C, --eta quantile score
    emp_sum as EMP_SM_C, --total employees (Nets 2015) within 1-mile of the station : NEEDS QUANTILE
    employee_score as EMP_SC_C, --employee count quantile score
    ipd_score as IPD_SC_C, --IPD within 1 mile (max value) --n/a (0), well below average (1), below average (2), average (3), abover average (4), well above average (5) 
    zerocar_sum as ZVH_SM_C, --Zero Vehicle Households (tract-level ASC 2019)  within 1-mile of the station : NEEDS QUANTILE
    zerocar_score as ZVH_SC_C, --Zero car quantile score
    pop_sum as POP_SM_C, --Total Population (tract-level ASC 2019) within 1-mile of the station : NEEDS QUANTILE
    pop_score as POP_SC_C, --Total population quantile score
    surv_count_C, --Count of most recent license plates within 2 miles of station
    tot_surv_count_C, --Total count of most recent license plate survey for station
    surv_calc as LPS_VA_C, --2 mile license count/Total Count 
    surv_score as LPS_SC_C, --Survey calculation quantile score
    crash_tot_C, --Total 5 year crashes within 2 miles of station
    ksi_tot as KSI_SM_C, --Total 5 year KSI crashes within 2 miles of station
    ksi_score as KSI_SC_C, --Total 5 year KSI quantile score
    ksi_bike_tot_C, --Total 5 year KSI bike crashes within 2 miles of station
    ksi_ped_tot_C, --Total 5 year KSI pedestrian crashes within 2 miles of station
    circuit_score as CIR_SC_C, --On/Adjacent within 1/4 mile (5) , <=1/2 (4), 1/2 to 1 mile (3), 1-2 miles (2), > 2miles (1) 
    inter_den as INT_VA_C, --total number of legs from intesection layer that is within 1 mile of station divided by area(acres)
    inter_score as INT_SC_C, --intersection density quantile score
    lts_miles as LTS_VA_C, --miles of lts island in 2 mile buffer by station_id
    lts_score as LTS_SC_C, --lts quantile score
    ped_miles as PED_VA_C, --miles of ped network in 2 mile buffer by station_id
    ped_score as PED_SC_C, --ped network quantile score
    ttv as TVV_VA_C, --total tvv
    tvv_score as TVV_SC_C, --tvv quantile score
    boards as NPBOARDS_VA_C,--npboards value
    parked as NPPARKED_VA_C,--npboards parked value
    percent as NPPERCENT_VA_C,--npboards percent value
    npb_sc as NPBOARDS_SC_C --npboards score
from
	rs_cycle_buffer_new ra
    ...
    left join tvvscore on tvvscore.dvrpc_id = ra.dvrpc_id
    left join npboards on npboards.dvrpc_id = ra.dvrpc_id
    order by dvrpc_id